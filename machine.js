...
export let onEvent = makeEvent.bind(transitionType);
...
let invokeFnType = {
  enter(machine2, service, event) {
    ...
    rn.then((data) => {
      if (machine2 === service.machine) return service.send({ type: 'done', data });
    }).catch((error) => {
      if (machine2 === service.machine) return service.send({ type: 'error', error });
    });
    return machine2;
  },
};
...
export let nested = (to, states) => invoke(createMachine(states, identity), transition('done', to));
